...
//
// 2015: Modified by Zak Estrada to dump nm-style output for functions
//       using the '-nm' flag. Originally intended to generate a sort
//       of System.map for windows kernel functions
//       Modifictions are tagged by /*BEGIN MOD*/ and /*END MOD*/
...
bool ParseArg(int argc, wchar_t *argv[])
{
  ...
  /*BEGIN MOD*/
  else if (!_wcsicmp(argv[0], L"-nm")) {
    iCount = 1;
    bReturn = bReturn && DumpFunctionsNm(g_pDiaSession);
    argc -= iCount;
    bReturn = bReturn && ParseArg(argc, &argv[iCount]);
  }
  /*END MOD*/
...
void PrintHelpOptions()
{
  ...
  /*BEGIN MOD*/
											L"  -nm : output functions in nm style output\n";
  /*END MOD*/
...
/*BEGIN MOD*/
////////////////////////////////////////////////////////////
// Dump all the functions in an nm-style format
//
bool DumpFunctionsNm(IDiaSession *pSession) {
  ...
  if (FAILED(pSession->getSymbolsByAddr(&pEnumSymbolsByAddr))){
	  printf("Couldn't get symbolsByAddr!\n");
      return false;
  }

  if (FAILED(pEnumSymbolsByAddr->symbolByAddr(1, 0, &pSymbol))) {
      printf("Couldn't get first symbol!\n");
      return false;
  }
  ...
  do {
	...
	if (pSymbol->get_function(&isFunc) == S_OK) {
		if(isFunc) {
			if (pSymbol->get_virtualAddress(&addr) == S_OK) {
				printf("%p ", (void *) addr);
			}

			if (pSymbol->get_scoped(&isGlobal) == S_OK) {
				if (isGlobal)
					putchar('T' ^ (isGlobal<<7)); //yay ASCII
			}
			else {
				//Assume global?
				putchar('T');
